...
int main(int argc, char* argv[]){
  ...
  if(!isLogScale){
    ...
    std::vector<SuperInteraction> lattices;   //possibility to store many lattice obj.
...
#     pragma omp single                  //only do this once in total.
      for(int i = 0; i < noLattice; ++i){
        ...
        SuperInteraction tmp(X,Y,Z,nParticles,seed,isBoundaryFix,
                             def.interactionStrength);
